...
while p.isConnected():
	...
	ball_vz = p.getBaseVelocity(elastic_ball)[0][2]

	turtle_pre = get_turtle_velocity(turtle)
	Time_limit = 2

	object_dist = np.sqrt(
		(turtle_z0[0] - ball_x)**2 + (turtle_z0[1] - ball_y)**2)
	...
	if object_dist >= 2:
		...
		feasible, output_right, output_left = Optimalcontrol_approach(
			turtle_z0, turtle_pre, ball_x, ball_y, Time_limit)
	else:
		...
		if T_upper > 0.5:
			feasible, output_right, output_left = Optimalcontrol(
				turtle_z0, turtle_pre, ball_x, ball_y, T_lower, T_upper)
